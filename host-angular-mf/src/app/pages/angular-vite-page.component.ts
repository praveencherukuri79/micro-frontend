...
export class AngularVitePageComponent implements AfterViewInit, OnDestroy {
  ...
  private async loadAndMountRemote(): Promise<void> {
    ...
    try {
      ...
      const remote = await import('angularVite/App');
      ...
      console.error('Error loading Angular Vite remote:', errorMsg);
      this.error = `Failed to load Angular Vite remote: ${errorMsg}`;
      this.loading = false;
    }
  }
}
